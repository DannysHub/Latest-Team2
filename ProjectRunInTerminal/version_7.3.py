...
def popup_mode_selection(title="Mode Selection"):
    ...
    while True:
        ...
        cv2.putText(frm, "W/S: move   Enter: select   Esc: quit",
                    (30, 280), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (180, 180, 180), 1)
        cv2.imshow(title, frm)
        k = cv2.waitKey(100) & 0xFF
        if k == 27:
            cv2.destroyAllWindows()
            sys.exit(0)
        if k in (ord('w'), ord('W'), 82): idx = (idx - 1) % len(opts)
        if k in (ord('s'), ord('S'), 84): idx = (idx + 1) % len(opts)
        if k in (13, 10):
...
def setup_client():
    while True:
        ...
        while True:
            ...
            cv2.putText(frm, "Enter:OK   Back:Esc   Del:Backspace", (20, 180),
...
def wait_both_ready(name, sock, is_host, network, lines, timeout=120):
    ...
    while True:
        ...
        for i, t in enumerate(lines):
            cv2.putText(frame, t, (20, 60 + i*40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
...
def main():
    ...
    if mode == 2:  # Host
        sock = setup_host()
        if sock is None:  # 按 B 返回
            return main()
        is_host = True
        network = True
    elif mode == 3:  # Client
        sock = setup_client()
        if sock is None:  # Back or ESC
            return main()
        is_host = False
        network = True
    ...
    while True:
        ...
        cont = show_next_round_popup(
            WINDOW_NAME, rnd, score,
            last_res or "", last_best or 0.0,
        ...
        while time.time() - start_prep < PREP_SECONDS:
            ...
            cv2.putText(frame, f"Ready: {remain}s", (20, 80),
        ...
        player_gesture = "None"
        start_cap = time.time()
        while time.time() - start_cap < CAPTURE_SECONDS:
            ret, frame = cap.read()
            frame = cv2.flip(frame, 1)
            ...
            if res.multi_hand_landmarks:
                ...
                if base and has_template(base):
                    ...
                    for tip, dval in devs.items():
                        ...
                        cv2.putText(frame, f"{tip}:{dval:.3f}", (20, y0),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
                        y0 += 20
                total_val = total_score(g, shape_val, open_val)
                buf.append(total_val)
                avg = sum(buf)/len(buf)
                if avg > best_total:
                    best_total, best_shape, best_open = avg, shape_val, open_val

                cv2.putText(frame, f"Shape:{shape_val:.1f}% Open:{open_val:.1f}% Total:{avg:.1f}%",
                            (20,140), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255,255,255), 2)
        ...
        if player_gesture in ("rock","paper","scissors"):
            if network:
                ...
                opp = (highmode_ai_choice(player_gesture, high_prob)
                       if high_active else
                       assisted_ai_choice(player_gesture, assist_prob)
                       if assist_active else
        ...
        cv2.putText(frame, f"Opp: {opp}", (10,80),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2)
        cv2.putText(frame, res_text, (10,130),
        ...
        append_csv(rnd,
                   ...
                   score, wins, losses, draws,
                   high_active, high_prob,
                   assist_active, assist_prob)
        last_res  = res_text
