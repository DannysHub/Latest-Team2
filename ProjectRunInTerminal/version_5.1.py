...
# 弹出下一局准备弹窗（本地或联机）
def show_next_round_popup(upcoming_round: int,
                          last_result: Optional[str],
                          last_best_total: Optional[float],
                          remote_mode: bool, sock, is_host: bool) -> bool:
    lines: List[str] = [f"Round {upcoming_round}"]
    if upcoming_round > 1:
        if last_result:
    ...
    lines.append(READY_MSG_LOCAL)
    return wait_both_ready(WINDOW_NAME, sock, is_host, remote_mode,
                           label=f"Round {upcoming_round}",
                           extra_lines=lines)
