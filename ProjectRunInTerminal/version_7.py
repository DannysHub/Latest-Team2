...
def setup_host():
    ...
    try:
        ...
        while waiting:
            ...
            cv2.putText(frame, "Waiting for Connection...", (30, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
            cv2.putText(frame, f"IP: {ip}:{PORT}", (30, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.imshow("Waiting for Client", frame)

            srv.settimeout(0.5)
            try:
                conn, addr = srv.accept()
                print(f"[Host] Connected: {addr}")
                cv2.destroyWindow("Waiting for Client")
                return conn
...
def setup_client():
    ...
    while True:
        frame = np.zeros((200, 600, 3), dtype='uint8')
        prompt = "Enter Host IP:"
        cv2.putText(frame, prompt, (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)

        display_ip = ip_chars + ("_" if cursor_visible else "")
        cv2.putText(frame, display_ip, (30, 120), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,255), 2)

        if time.time() - last_cursor_toggle > 0.5:
            cursor_visible = not cursor_visible
            last_cursor_toggle = time.time()

        cv2.putText(frame, "Enter: confirm   Backspace: delete   ESC: cancel", (20, 180),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (180,180,180), 1)

        cv2.imshow("Enter Host IP", frame)
        key = cv2.waitKey(100)

        if key == 27:
            cv2.destroyWindow("Enter Host IP")
            sys.exit(0)
...
def show_next_round_popup(window_name, upcoming_round, current_score, last_result, last_best_total, is_network, sock, is_host,
                          timeout=120.0, extra_lines=None):
    READY_PROMPT = "Press ENTER to start"
    START_LABEL = f"Round {upcoming_round}"
    # 显示回合号和当前分数
    lines = [START_LABEL, f"Score: {current_score}"]
    if upcoming_round > 1:
        if last_result:
            res = ""
    ...
    if is_network and sock:
        ...
        return wait_both_ready(window_name, sock, is_host, True,
                               ...
                               timeout=timeout)
    while True:
        frame = np.zeros((480, 720, 3), dtype='uint8')
        y = 80
        for text in lines:
...
def popup_mode_selection(window_name="Mode Selection") -> str:
    import numpy as np
    options = ["Local (Single Player)", "Online (Host)", "Online (Client)"]
    index = 0

    while True:
        frame = np.zeros((400, 640, 3), dtype='uint8')
        cv2.putText(frame, "Select Mode:", (40, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 0), 2)

        for i, opt in enumerate(options):
            color = (0, 255, 255) if i == index else (255, 255, 255)
            cv2.putText(frame, f"> {opt}" if i == index else f"  {opt}", (60, 140 + i*60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)

        cv2.putText(frame, "W/S: move   Enter: select   Esc: quit", (40, 360), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (160,160,160), 1)
        cv2.imshow(window_name, frame)
